/**
 * All os/o ID should be exported from this file.
 */
export const entityMonth = "January";
export const entityDate = "22";
export const entityYear = "2025";
export const entityName = "Harmoni Istiglal X Bukhari Creative Group";
export const entityAlias1 = "Harmoni Istiqlal X Bukhari";
export const entityAlias2 = "Harmoni X Bukhari Creative Group";
export const entityAddress =
  "Perkantoran Masjid Istiqlal Ruang 17-18 (FBN, Jl. Veteran I, Gambir, Kecamatan Sawah Besar, Kota Jakarta Pusat, Daerah Khusus Ibukota Jakarta 10110, INDONESIA";
export const entityEmail = "partnership@bukharicreative.group";
